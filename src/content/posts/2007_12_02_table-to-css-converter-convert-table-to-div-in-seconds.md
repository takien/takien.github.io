---
title: "Table to CSS Converter, Convert Table to DIV in Seconds"
date: "2007-12-02T00:00:00Z"
categories: ["Website"]
tags: []
slug: "2007/12/02/table-to-css-converter-convert-table-to-div-in-seconds"
legacyUrls: ["189/table-to-css-converter-convert-table-to-div-in-seconds.php", "189/table-to-css-converter-convert-table-to-div-in-seconds", "blog/2007/12/02/table-to-css-converter-convert-table-to-div-in-seconds", "table-to-css-converter-convert-table-to-div-in-seconds"]
source: "takien.com"
comments: [
  {
    "author": "rizkyonline",
    "date": "",
    "text": "wah mantap om&#8230;\nsering2 ya sharing ilmu-nya&#8230;"
  },
  {
    "author": "iklan baris gratis",
    "date": "",
    "text": "Ada baiknya kalo belajar css.\nSelamat Berkarya"
  },
  {
    "author": "Whois Gratis Juga",
    "date": "",
    "text": "wah ini info top, cek UCP bro  \nSip, mempersingkat waktu nih. I love tools&#8230;."
  },
  {
    "author": "geblek",
    "date": "",
    "text": "mantap\nlebih mantap lagi kalau langsung jadi themes wordpress"
  },
  {
    "author": "dhit",
    "date": "",
    "text": "klw bisa full version, tp free, malah jadi MANTAB&#8230; trims ya"
  },
  {
    "author": "takien.com",
    "date": "",
    "text": "hehe.. ini aja dah mantab kok..\nemang ada file html yang lebih dari 25kb?\ngak ada kan&#8230;\nini cocok buat layout templet&#8230;"
  },
  {
    "author": "monza",
    "date": "",
    "text": "wah, top abis\ndengan adanya soft ini, bisa menghemat waktu kita..\napalagi buat pemula cs kayak aku (L)"
  },
  {
    "author": "Lutfi",
    "date": "",
    "text": "bagi dong passwordnya, kagak bisa di-ektrak nih   send email ya"
  },
  {
    "author": "Daniezz",
    "date": "",
    "text": "tengkyuuu bgt!!\nampe nangis2 nih bikin table css manual, gila div2nya banyak bgt&#8230;.PUSIIING ..hiks2"
  },
  {
    "author": "EdO",
    "date": "",
    "text": "wah&#8230;bener..bener&#8230;\nberbagi ilmu terus yeah&#8230;"
  },
  {
    "author": "akangaziz",
    "date": "",
    "text": "bos kok gak bisa ya pake kata sandi http://www.situskamu.com\nudah di ganti kah kata sandi nya?"
  },
  {
    "author": "@di",
    "date": "",
    "text": "Infonya oke banget. Pas lagi butuh ternyata ada di sini. Langsung sedot dan jalan lancar. Trims"
  },
  {
    "author": "ramdhan",
    "date": "",
    "text": "kok ga bisa si extract sih&#8230;\ntrus di isi pasword wrong2 mulu&#8230;"
  },
  {
    "author": "purba",
    "date": "",
    "text": "Thx bos.. sdh di download dan berhasil. GBU"
  },
  {
    "author": "radiaku",
    "date": "",
    "text": "masih versi trial ya bos. waduh"
  },
  {
    "author": "takien.com",
    "date": "",
    "text": "yup, tapi ini kan opsional..\nsebaiknya tetap biasakan pake css manual"
  },
  {
    "author": "takien.com",
    "date": "",
    "text": "kan ada tuh passwordnya, baca deh postingnya sampai habis"
  },
  {
    "author": "takien.com",
    "date": "",
    "text": "bisa kok&#8230;\ngak pake http:// tuh"
  },
  {
    "author": "takien.com",
    "date": "",
    "text": "sudah tersedia link download yang tidak pakai password, cek kembali posting di atas.\nthanks"
  }
]
---

<p><img src="/images/2007/12/div.png" alt="table to div converter" title="" class="rounded-lg my-6 max-w-full h-auto shadow-md" loading="lazy" /><em>How to convert table to div?</em> Sepertinya kalimat ini menjadi frequently asked question di forum-forum deh. Secara aku sendiri pernah nanya itu, tapi jawaban yang diberikan kurang atau bahkan tidak memuaskan. Ya, karena ujung-ujungnya kita disuruh belajar CSS.. <img src="/images/2007/12/icon_sad.gif" alt=":(" title="" class="rounded-lg my-6 max-w-full h-auto shadow-md" loading="lazy" />  cape deh. Mengedit CSS yang sudah ada memang mudah, tapi membuat sebuah layout website full css from scratch  bukanlah hal yang mudah, setidaknya bagi  pemula seperti yang punya blog ini. Bagaimana tidak, setelah sekian kali edit dan preview di Firefox, eh.. malah tampil kacau di Internet Explorer&#8230; huh&#8230; Terpaksa deh layout tableless nya ditundah dulu sampai</p>
<p>Tapi pagi ini ada sesuatu yang sedikit melegakan, lagi iseng-iseng nyari script,  eh.. malah nemu link Table to CSS converter&#8230; Hah, bukannya sekitar sebulan yang lalu aku pernah nyari program sejenis ini dan ternyata gak nemu? Oke deh, mungkin sebulan yang lalu program ini belum dibuat atau programmernya lagi sibuk ujian mid-semester kalee.. <img src="/images/2007/12/icon_biggrin.gif" alt=":D" title="" class="rounded-lg my-6 max-w-full h-auto shadow-md" loading="lazy" /><br/>
<span id="more-189"></span><br/>
Peduli amat, yang penting sekarang sudah ada, tunggu apa lagi? Langsung sedot. Loh, trial neh&#8230; <img src="/images/2007/12/icon_sad.gif" alt=":(" title="" class="rounded-lg my-6 max-w-full h-auto shadow-md" loading="lazy" /> ( Oops&#8230; lihat dulu apa bedanya trial sama full. Oh, iya ding. Trial hanya bisa melakukan konversi file HTML dengan ukuran maksimal 25kb, sementara yang full bisa lebih dari 25kb. Lha, kalau cuma begitu sih gak masalah dong, bukannya file-file HTML ku paling besar berukuran 10 kb. Wack, programmernya gak ngadain survey lapangan dulu sih sewaktu mau buat programmya, hehehehe&#8230;</p>
<p>Lho, kok malah cerita? Mana programmnyaaaaaaaa&#8230;. Ya ya.. ya&#8230; bentar napa&#8230;<br/>
Berikut ini hasil tangkapan layar nya.</p>
<figure class="image-missing-placeholder" role="img" aria-label="table-to-css-converter.png">
  <div class="placeholder-inner">
    <svg class="placeholder-icon" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <rect width="18" height="18" x="3" y="3" rx="2" ry="2"></rect>
      <circle cx="9" cy="9" r="2"></circle>
      <path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"></path>
    </svg>
    <span class="placeholder-text">table-to-css-converter.png</span>
  </div>
</figure>
<p>Keterangan:</p>
<ol>
<li>Source URL url or file, lokasi sumber file baik itu di komputer lokal maupun di url internet.</li>
<li>Output file, tentukan lokasi dan nama file hasil konversi.</li>
<li>Style type,
<ul>
<li>Embedded, kode css akan digabungkan dalam file dan didevinisikan di dengan tag style di antara tag header.</li>
<li>External, kode css akan diletakkan terpisah di  file  .css dalam di folder yang sama dengan file asli.</li>
<li>Inline, kode css langsung di sisipkan di setiap tag div.</li>
</ul>
<p>Rekomendasi pilih type External.</li>
<li>Prefered link type,
<ul>
<li>Absolute, link tetap.</li>
<li>Relatif, link akan menyesuaikan dengan lokasi file.</li>
</ul>
</li>
<li>Yang lainnya biarkan dengan nilai default.</li>
<li>Klik Convert untuk memulai konversi. Done! <img src="/images/2007/12/icon_smile.gif" alt=":)" title="" class="rounded-lg my-6 max-w-full h-auto shadow-md" loading="lazy" /> </li>
</ol>
<p>Download programnya disini, sandi: www.situskamu.com<br/>
Kok pakai password segala? Biar keren dunk. ^_^<br/>
<a title="table2css-2013-trial.rar" href="http://takien.com/wp-content/uploads/2007/12/table2css-2013-trial.rar">table2css-2013-trial.rar</a></p>
<p><strong>Download</strong><br/>
Karena banyak yang kesusahan kalau pakai password,<br/>
saya upload yang tidak pake password di Ziddu<br/>
Silahkan di download melalui link berikut</p>
<p><a href="http://www.ziddu.com/download/3521701/table2css-2.0.13-trial.zip.html">http://www.ziddu.com/download/3521701/table2css-2.0.13-trial.zip.html</a></p>
